...
Benchmarking using profvis is not so easy due to recursion but Rprof in base
works fine:

```{r, eval = FALSE}
Rprof(tmp <- tempfile())
solve(sudoku, findChoices)
Rprof()
summaryRprof(tmp)
```

## Some cleaning for benchmarking

The `%<-%` operator in zeallot brings a bit of overhead. So elimate that first,
code is shown in the next slides. It is not really important and only shown for
completeness, the unpacking is just done manually.
